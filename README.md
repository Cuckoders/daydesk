...
![Rust tests](https://img.shields.io/badge/Rust_tests-41%2F41-success)
...
- двусторонняя синхронизация задач между desktop и mobile каждые 30 секунд, с ручным запуском и офлайн-очередью;
...
Настольный клиент автоматически обменивается задачами каждые 30 секунд. Изменения, сделанные без сети, остаются в локальной очереди; при первом подключении локальные и серверные задачи объединяются без удаления записей, существующих только на одном устройстве. Точные правила произвольного повтора desktop-задач и отложенное время напоминания сохраняются в общем протоколе.
