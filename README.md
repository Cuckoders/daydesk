...
Мобильный клиент подключает Gmail и Outlook через OAuth 2.0 с PKCE в системном браузере, а Yandex, Mail.ru, iCloud и другие ящики — через IMAP. Sync-сервис шифрует OAuth-токены и пароли AES-256-GCM; телефон не сохраняет секреты или тексты писем в AsyncStorage. Reader отображает только обычный текст без выполнения HTML и загрузки пикселей отслеживания.
