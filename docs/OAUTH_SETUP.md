...
2. Включите [Gmail API](https://console.cloud.google.com/apis/library/gmail.googleapis.com).
...
DayDesk запрашивает два почтовых scope: `https://www.googleapis.com/auth/gmail.readonly` для чтения и `https://www.googleapis.com/auth/gmail.send` для отправки. Доступ к изменению или удалению писем не запрашивается.
...
5. Добавьте delegated permissions Microsoft Graph: `User.Read`, `Mail.Read` и `Mail.Send`.
...
Если аккаунт подключался к сборке DayDesk, которая умела только читать почту, отключите и подключите его снова. Старый refresh token не содержит разрешения на отправку.
...
- в локальном состоянии интерфейса остаются только адрес аккаунта, название провайдера и данные списка писем;
